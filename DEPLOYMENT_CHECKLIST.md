...
> Les workflows **`ci-server.yml`** et **`ci-client.yml`** suivent des **stages DevOps** (qualité → tests → build → package Docker). Le **CD** attend **les deux** CI vertes sur le même commit avant de déployer.
